...
class Simple_login {
	...
	*/
	...
	public function login($username, $password) {
		...
		$query = $this->CI->db->get_where('users',array('username'=>$username,'password' => md5($password))); //md5($password)
		 
		if($query->num_rows() == 1) {
			//ambil data user berdasar username
			$row  = $this->CI->db->query('SELECT * FROM users where username = "'.$username.'"'); //selsct all *
			$admin     = $row->row();
			$id   = $admin->username;
			$nama   = $admin->nama;
			...
			$this->CI->session->set_userdata('id_login', uniqid(rand())); //login dibanyak PC, agar unik
